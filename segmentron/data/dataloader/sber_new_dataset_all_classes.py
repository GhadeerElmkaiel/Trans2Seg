...
class SberNewSegmentationAll(SegmentationDataset):
    ...
    def __init__(self, root='datasets/Sber3500', split='test', mode=None, transform=None, **kwargs):
        super(SberNewSegmentationAll, self).__init__(root, split, mode, transform, **kwargs)
        root = os.path.join(self.root, self.BASE_DIR)
        assert os.path.exists(root), "Please put the data in {SEG_ROOT}/datasets/Sber3500"
        self.images, self.masks = _get_sber3500_pairs(root, split)
...
def _get_sber3500_pairs(folder, mode='train'):
    img_paths = []
    mask_paths = []
    if mode == 'train':
...
if __name__ == '__main__':
    train_dataset = SberNewSegmentationAll()
