...
class SberMergedSegmentationAll(SegmentationDataset):
    ...
    def __init__(self, root='datasets/SberMerged', split='test', mode=None, transform=None, **kwargs):
        super(SberMergedSegmentationAll, self).__init__(root, split, mode, transform, **kwargs)
        root = os.path.join(self.root, self.BASE_DIR)
        assert os.path.exists(root), "Please put the data in {SEG_ROOT}/datasets/SberMerged"
        self.images, self.masks = _get_sbermerged_pairs(root, split)
...
def _get_sbermerged_pairs(folder, mode='train'):
    img_paths = []
    mask_paths = []
    if mode == 'train':
        img_folder = os.path.join(folder, 'train/images')
        mask_folder = os.path.join(folder, 'train/Semantic')
    elif mode == "val":
        img_folder = os.path.join(folder, 'validation/images')
        mask_folder = os.path.join(folder, 'validation/Semantic')
    else:
        assert  mode == "test"
        img_folder = os.path.join(folder, 'test/images')
        mask_folder = os.path.join(folder, 'test/Semantic')
...
if __name__ == '__main__':
    train_dataset = SberMergedSegmentationAll()
