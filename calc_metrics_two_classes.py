...
# res_root = "runs/visual/Sber2400_50_all_classes/"
res_root = "runs/visual/Sber2400_50_all_optical_all_floor/"
...
path_to_testing = path_to_target_flipped
...
for name in images:
    ...
    trans_all_floor = trans_arr==1
    # trans_mirror = trans_arr==6
    # trans_oos = trans_arr==6
    # trans_fu = trans_arr==6
    ...
    ##############################################
    # Only Glass
    intersection_glass = np.sum(np.logical_and(glass_arr, trans_optical))
    union_glass = np.sum(np.logical_or(glass_arr, trans_optical))
    acc_all = np.sum(np.logical_not(np.logical_xor(glass_arr, trans_optical)))
    if union_glass == 0:
        iou_glass = 1
    else:
    ...
    ##############################################
    # Glass and Mirror
    intersection_G_M = np.sum(np.logical_and(G_M_arr, trans_optical))
    union_G_M = np.sum(np.logical_or(G_M_arr, trans_optical))
    acc_G_M = np.sum(np.logical_not(np.logical_xor(G_M_arr, trans_optical)))
    if union_G_M == 0:
        iou_G_M = 1
    else:
